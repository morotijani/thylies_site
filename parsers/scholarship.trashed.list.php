...
$total_data = $conn->query("SELECT * FROM thylies_scholarship WHERE trash = 1")->rowCount();
...
if ($total_data > 0) {
	...
	foreach ($result as $row) {
		...
		if ($conn->query("SELECT * FROM thylies_transactions WHERE scholarship_id = '".$scholarship_id."' AND transaction_service = 'scholarship' ")->rowCount() > 0) {
			$paid = 'success';
		}
